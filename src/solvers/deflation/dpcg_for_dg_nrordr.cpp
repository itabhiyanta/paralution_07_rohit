...
template <class OperatorType, class VectorType, typename ValueType>
void DPCG_FOR_DG<OperatorType, VectorType, ValueType>::MakeR_CSR(const int nrows, const int ncols, const int m) {

  //FILE *fp, *fp1;
  int i,j;
  int *R_row_offset = NULL;  int *R_col = NULL;  ValueType *R_val = NULL;
  
  this->R_.AllocateCSR("R",nrows,nrows,ncols);
  
  this->R_.LeaveDataPtrCSR(&R_row_offset, &R_col, &R_val);
  ...
  for(i=0, j=0 ; i<nrows; i++)
  {
        R_col[j]=m*i;
        R_val[j] = 1.0f;
        R_row_offset[i+1]=R_row_offset[i]+1;
        j++;
  }
  R_row_offset[i]=nrows;
  this->R_.SetDataPtrCSR(&R_row_offset, &R_col, &R_val, "R", nrows, nrows, ncols);
}
...
#ifdef DPCG_INNR  
  this->ls_inner_.SetNVectors_eachdirec2D(DEFVEX_PERDIREC, DEFVEX_PERDIREC);
  this->ls_inner_.Set_alldims2D(sqrt(this->A0_nrows_), sqrt(this->A0_nrows_));
  this->ls_inner_.MakeZ_CSR_2D();
  Jacobi<LocalMatrix<ValueType>, LocalVector<ValueType>, ValueType > *j_p;
  j_p= new Jacobi<LocalMatrix<ValueType>, LocalVector<ValueType>, ValueType>;
  this->precond_inner_ = j_p;
#endif
...
// 
    ...
    cout<<"Time in solving innr system"<<time_innrsolve<<" secs"<<endl;
